...
type OurTimeType = Maybe ([DSTType], String)
month      :: String -> OurTimeType
namedMonth :: String -> OurTimeType
day        :: String -> OurTimeType
namedDay   :: String -> OurTimeType
year       :: String -> OurTimeType
constant   :: String -> String -> OurTimeType
fseqm      :: (String -> OurTimeType)
           -> (String -> OurTimeType)
           -> String
           -> OurTimeType
try        :: (String -> OurTimeType)
           -> (String -> OurTimeType)
           -> String -> OurTimeType
end        :: String -> OurTimeType
...
month s | "0" <= s && s <= "99" = Just $ ([m], [])
  where
    m = MonthV $ monthList !! ((read s) `mod` 12)
    monthList =
      [December, January, February, March, April, May, June,
       July, August, September, October, November]
month _ = Nothing

namedMonth s | s == "January"   = Just ([MonthV January], "")
             | s == "February"  = Just ([MonthV February], "")
             | s == "March"     = Just ([MonthV March], "")
             | s == "April"     = Just ([MonthV April], "")
             | s == "May"       = Just ([MonthV May], "")
             | s == "June"      = Just ([MonthV June], "")
             | s == "July"      = Just ([MonthV July], "")
             | s == "August"    = Just ([MonthV August], "")
             | s == "September" = Just ([MonthV September], "")
             | s == "October"   = Just ([MonthV October], "")
             | s == "November"  = Just ([MonthV November], "")
             | s == "December"  = Just ([MonthV December], "")
             | otherwise        = Nothing

day s | "0" <= s && s <= "99" = Just $ ([d], [])
  where d = DayV (read s)
day _ = Nothing

namedDay s | s == "Monday"    = Just ([DayV 01], "")
           | s == "Tuesday"   = Just ([DayV 02], "")
           | s == "Wednesday" = Just ([DayV 03], "")
           | s == "Thursday"  = Just ([DayV 04], "")
           | s == "Friday"    = Just ([DayV 05], "")
           | s == "Saturday"  = Just ([DayV 06], "")
           | s == "Sunday"    = Just ([DayV 07], "")
           | otherwise        = Nothing
           
year s | "0" <= s && s <= "9999" = Just $ ([d], [])
  where d = Year (read s)
year _ = Nothing

constant x s | x == s = Just ([], "")
constant _ _ = Nothing

fseqm fmt1 fmt2 s | fmt2 s == Nothing = fmt1 s
                  | fmt1 s == Nothing = fmt2 s
                  | otherwise = do
                      ret2 <- fmt2 s
                      ret1 <- fmt1 (snd ret2)
                      return $ (fst ret1 ++ fst ret2, snd ret1)

try fmt1 fmt2 s | fmt1 s == Nothing = fmt2 s
                | otherwise         = fmt1 s
...
-- japaneseDate :: String -> OurTimeType
-- japaneseDate = fseqm (fseqm year (constant "-")) (fseqm (fseqm month (constant "-")) day)
...
--longDates :: String -> OurTimeType
--longDates = fseqm (fseqm namedDay (constant ",")) (fseqm namedMonth (fseqm day (constant ",")
